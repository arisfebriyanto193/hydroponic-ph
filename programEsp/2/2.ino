...
// ---- State Machine pH Control ----
enum PhControlState {
  PH_STATE_STANDBY,   // Menunggu, siap cek pH
  PH_STATE_DOSING,    // Relay aktif, sedang injeksi
  PH_STATE_MIXING     // Relay mati, menunggu cairan larut
};
PhControlState phCtrlState  = PH_STATE_STANDBY;
unsigned long  stateStartMs = 0;    // Waktu masuk state saat ini
int            dosingCounter = 0;   // Jumlah injeksi berurutan (untuk failsafe)
...
void setupServerRoutes() {
  ...
  server.on("/api/status", HTTP_GET, []() {
    float ph  = readPH();
    float tds = readTDS();
    // Map enum ke string untuk dashboard
    const char* stateStr = (phCtrlState == PH_STATE_DOSING) ? "dosing"
                         : (phCtrlState == PH_STATE_MIXING) ? "mixing"
                         : "standby";
    String j = "{\"ph\":" + String(ph,2) + ",\"threshold\":" + String(threshold,2) +
               ...
               ",\"tds\":" + String(tds,1) +
               ",\"ctrl_state\":\"" + stateStr + "\""+
               ",\"dosing_count\":" + String(dosingCounter) + "}";
    server.send(200, "application/json", j);
  });
    ...
    if (server.hasArg("target")) {
      ...
      if (tgt == "relay1" && currentMode == "manual") { setRelay1(!relay1State); }
        ...
        if (currentMode == "manual") {
          ...
          phCtrlState = PH_STATE_STANDBY; // reset state machine
        } else {
          // Masuk ke mode otomatis: reset state machine & counter
          phCtrlState  = PH_STATE_STANDBY;
          stateStartMs = millis();
          resetDosingCounter();
        }
      }
...
// ====================== pH State Machine Control ====================
//
//  STANDBY ──(pH di luar batas)──> DOSING  (relay ON selama DOSING_TIME_MS)
//  DOSING  ──(timer habis)───────> MIXING  (relay OFF, tunggu MIXING_DELAY_MS)
//  MIXING  ──(timer habis)───────> STANDBY (cek pH ulang)
//
//  Failsafe: dosingCounter >= DOSING_FAILSAFE_COUNT → paksa manual, relay OFF
//  Reset counter: pH kembali ke range normal saat STANDBY
// ====================================================================
void resetDosingCounter() {
  if (dosingCounter != 0) {
    dosingCounter = 0;
    Serial.println("[AutoCtrl] Dosing counter direset.");
  }
}
...
void autoModeControl(float ph) {
  ...
  switch (phCtrlState) {
    ...
    // ------------------------------------------------------------------
    case PH_STATE_STANDBY: {
      bool phTooHigh = (ph > threshold + phDeadband);
      bool phTooLow  = (ph < threshold - phDeadband);

      // pH sudah kembali normal → reset counter failsafe
      if (!phTooHigh && !phTooLow) {
        resetDosingCounter();
        break;
      }

      // ---- Failsafe: terlalu banyak injeksi berurutan ----
      if (dosingCounter >= DOSING_FAILSAFE_COUNT) {
        Serial.printf("[AutoCtrl] ⚠ FAILSAFE! %d injeksi berurutan, pH belum normal. "
                      "Beralih ke mode manual.\n", dosingCounter);
        currentMode = "manual";
        setRelay1(false);
        setRelay2(false);
        phCtrlState = PH_STATE_STANDBY;
        // Kirim update mode ke WebSocket
        if (wsConnected && !isAPMode) {
          StaticJsonDocument<160> doc;
          doc["action"]  = "publish";
          doc["topic"]   = String("data/mode/user/") + cfgUserId;
          doc["payload"] = "manual";
          String msg; serializeJson(doc, msg);
          webSocket.sendTXT(msg);
        }
      ...
      // ---- Mulai fase injeksi (DOSING) ----
      if (phTooHigh) {
        Serial.printf("[AutoCtrl] pH %.2f > %.2f+%.2f → Injeksi ASAM (Relay1 ON, %d detik). Counter: %d\n",
                      ph, threshold, phDeadband, DOSING_TIME_MS / 1000, dosingCounter + 1);
        setRelay1(true);
        setRelay2(false);   // pastikan relay basa mati
      } else { // phTooLow
        Serial.printf("[AutoCtrl] pH %.2f < %.2f-%.2f → Injeksi BASA (Relay2 ON, %d detik). Counter: %d\n",
                      ph, threshold, phDeadband, DOSING_TIME_MS / 1000, dosingCounter + 1);
        setRelay2(true);
        setRelay1(false);   // pastikan relay asam mati
      }
      dosingCounter++;
      stateStartMs = now;
      phCtrlState  = PH_STATE_DOSING;
      break;
    }

    // ------------------------------------------------------------------
    case PH_STATE_DOSING: {
      // Tunggu sampai DOSING_TIME_MS berlalu, lalu paksa relay mati
      if (now - stateStartMs >= DOSING_TIME_MS) {
        setRelay1(false);
        setRelay2(false);
        Serial.printf("[AutoCtrl] Injeksi selesai. Masuk fase MIXING (%d detik).\n",
                      MIXING_DELAY_MS / 1000);
        stateStartMs = now;
        phCtrlState  = PH_STATE_MIXING;
      }
      // Selama DOSING: abaikan pembacaan pH, tunggu saja
      break;
    }

    // ------------------------------------------------------------------
    case PH_STATE_MIXING: {
      // Tunggu cairan larut merata (MIXING_DELAY_MS)
      if (now - stateStartMs >= MIXING_DELAY_MS) {
        Serial.println("[AutoCtrl] Mixing selesai. Kembali ke STANDBY, pH akan dicek ulang.");
        phCtrlState = PH_STATE_STANDBY;
      }
      // Selama MIXING: relay sudah mati, abaikan pH sementara
      break;
    }
  }
}
  ...
  if (topic == topicRelay1) {
    ...
    if (newMode == "otomatis" || newMode == "manual") {
      ...
      if (currentMode == "manual") {
        ...
        if (relay2State) setRelay2(false);
        phCtrlState = PH_STATE_STANDBY; // reset state machine
      } else if (currentMode == "otomatis") {
        // Reset state machine & counter saat masuk mode otomatis
        phCtrlState  = PH_STATE_STANDBY;
        stateStartMs = millis();
        resetDosingCounter();
      }
